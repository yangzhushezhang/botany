        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            if ($data_json && $data_json['status'] == 0) {
                ...
                for ($j = 0; $j < 5; $j++) {
                    ...
                    $client_http->setHeaders($headers, false, false);
                    $data = '{"farmId":"' . $data_json['data']['_id'] . '","toolId":2,"token":{"challenge":"default","seccode":"default","validate":"default"}}';
                    $client_http->setTimeout(5);
                    $client_http->setConnectTimeout(10);
                    $response = $client_http->post($data);
                ...
                Tools::WriteLogger($this->data['user_id'], 2, "进程 PutPotProcess 放大花盆失败" . $response, $this->data['account_number_id'], 2, $data_json['data']['_id']);
                return true;
            }
        }
