        ...
        try {
            for ($i = 0; $i < 5; $i++) {
                ...
                if ($data && $data['status'] == 0) {   #返回了 0
                    AccountNumberModel::create()->where(['id' => $account_number_id])->update(['status' => 1]);  #  获取农场信息失败
            ...
            # 跟新状态状态
            AccountNumberModel::create()->where(['id' => $account_number_id])->update(['status' => 3]);  #  获取农场信息失败
