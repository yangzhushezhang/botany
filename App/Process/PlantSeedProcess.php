                    ...
                    if ($id) {
                        DbManager::getInstance()->invoke(function ($client) use ($id, $redis) {
                            ...
                            if (count($id_array) == 3) {     # account_number_id  种子类型 user_id
                                ...
                                if (!$one) {
                                    Tools::WriteLogger($id_array[2], 2, "PlantSeedProcess 账户id:" . $id_array[1] . "不存在 ");
                                    return false;
                                }
                                ...
                                if (!$data) {
                                    # 解析失败 收获失败
                                    Tools::WriteLogger($id_array[2], 2, "账户id:" . $id_array[1] . "种植失败  json 解析失败");
                                    return false;
                                }

                                if ($data['status'] != 0) {
                                    Tools::WriteLogger($id_array[2], 2, "账户id:" . $id_array[1] . " 种植失败.." . $result);
                                    return false;
                                }
                                ...
                                if (!$res) {
                                    Tools::WriteLogger($id_array[2], 2, "账户id:" . $id_array[1] . " PlantSeedProcess 插入数据库失败" . $result);
                                }
                                #需要 去放小盆
